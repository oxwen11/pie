...
- `.agents/skills/react-doctor` — React health check; `doctor.config.json` opts into the strict SPA subset (0.9.14+); CI fails on error-level only. Sibling skills: `performance` (`scan` on `:4190`), `improve-react` (read-only audit/plans)
