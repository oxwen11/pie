...
Runtime UI checks use `.agents/skills/verify-pie` (isolated launch, doctor,
drive, evidence, cleanup; `.cursor/skills/verify-pie` is a symlink).
`.agents/skills/verify` is the short two-process note.
...
- `.agents/skills/verify-pie` — isolated launch/doctor/drive/cleanup for the web UI (linked from `.cursor/skills`, `.claude/skills`, `.codex/skills`)
- `.agents/skills/verify-pie-cli` — isolated `pie` / `pie daemon` / `pie serve` recipe; implementation is `tools/verify-pie-cli` (`@getpie/verify-pie-cli`)
- `.agents/skills/verify-pie-desktop` — isolated Electron + token daemon
