...
|                                 |                                                                    |
| ------------------------------- | ------------------------------------------------------------------ |
| `pnpm test`                     | `turbo run test`; one package: `pnpm --filter @getpie/server test` |
...
`format` is root-only (oxfmt) and not a turbo task. `test` / `lint` /
`lint:check` go through turbo so they wait on `^build` or
`@getpie/oxlint#build`. `test` and `typecheck` are cached, so re-run with
`--force` after changing something outside their hash inputs. Server/CLI/
desktop `build` stays uncached because those artifacts embed HEAD as the
daemon compatibility key. `pnpm clean` runs `turbo run clean` then
