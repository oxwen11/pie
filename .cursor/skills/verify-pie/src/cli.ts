...
const usageText = `Usage:
  verify-pie launch [--replace]
  verify-pie doctor
  verify-pie evidence path|init|screenshot|snapshot|url|side-effects|note
  verify-pie cleanup [run-dir]
...
try {
  ...
  console.error(message.startsWith("verify-pie") ? message : `verify-pie: ${message}`);
