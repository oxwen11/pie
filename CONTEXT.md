...
**Pi RPC child**:
Always Bun: `bun <pi-rpc.js> --mode rpc …`. `@getpie/server#build` emits the JS with `bun build --target bun`. Value imports go through `harness/pi/pi-sdk.ts` (not the package barrel). A two-file pnpm patch on the SDK only touches the extension loader (do not namespace-import the TUI barrel) and builtin theme JSON (inline, so the child needs no theme copy). Unpackaged / CLI look up `bun` on PATH. Packaged desktop ships Bun (`extraResources/vendor/bun`, currently `bun-v1.4.2`) and sets `PIE_BUN` / `PIE_PI_EXECUTABLE` to the extraResources copies. Missing Bun fails availability.
