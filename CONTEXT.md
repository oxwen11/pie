...
**Settings** (`$PIE_HOME/config.json`):
Agent-facing pie preferences in one JSON document with three owner objects: `ui` (SPA — `ui.theme` today), `desktop` (Electron Main — `desktop.window`), and `agent` (pie-owned operator prefs about the agent; no keys until one exists). Distinct from Project/Session JSON under `storage/` (no `{ version, data }` envelope) and from Pi's own settings files. Named only in `packages/server/src/config/paths.ts` (`configFile`). Theme is server-owned — CLI `serve` and the desktop renderer call `settings.get`/`settings.update`. Window bounds are read and written by desktop Main with direct file I/O so the window can restore before the daemon is up. Each writer merges its slice and leaves sibling objects in place. Leftover `ui.window` is read as `desktop.window` and relocated on the next save.
_Avoid_: putting operator prefs in `projects.json`; wrapping this file in the JSON store `{ version, data }` envelope; writing Pi's settings files; a second settings file; Electron `userData` for prefs or window bounds; a canonical rewrite that drops sibling objects; putting `desktop.window` on the RPC wire; writing an empty `agent` object; proxying Pi model defaults through `agent`; TOML for this file (agents consume JSON; humans are not the reader)
