...
**pie-pi-process**:
Always Bun: `bun <pi-process.js> --mode rpc …`. `@getpie/server#build` emits the JS with `bun build --target bun`. A pnpm patch keeps extension UI components and `pi-tui` on the package barrel / virtualModules, drops InteractiveMode, inlines builtin theme JSON, and no-ops highlight.js. Unpackaged / CLI look up `bun` on PATH. Packaged desktop ships Bun (`extraResources/vendor/bun`, currently `bun-v1.4.2`) and sets `PIE_BUN` / `PIE_PI_EXECUTABLE` to the extraResources copies. Missing Bun fails availability. Bundled `@ff-labs/pi-fff` ships as a JS island next to pie-pi-process; the platform native lib is downloaded in the background on **server start** (Desktop launch, `pie serve`, `pie daemon start`, and any CLI command that spawns the local daemon) into `$PIE_HOME/vendor/fff/`. It is not in the Desktop installer. Desktop and CLI share that cache. Until the lib is ready, Pi's built-in `find` / `grep` stay. `PIE_FFF=0` turns the extension off.
