...
**PIE_DAEMON_RUNTIME**:
Desktop-only switch for the _daemon_ (not the Pi child). Desktop sets `node` so the daemon is Electron-as-Node (`Pie Helper` + asar `server.mjs`) and the terminal uses `node-pty`. Unset / `node` is the same. `bun` still exists as a manual override (`PIE_BUN` or PATH `bun` + `server.mjs`, no `ELECTRON_RUN_AS_NODE`) but is not the product path. Restart the app and stop any existing daemon (`pie daemon stop`) after flipping — attach would otherwise reuse the previous runtime.
_Avoid_: spawning the shebang `pi` binary under Bun; treating this as a replacement for idle soft-close / reclaim; using a user-installed `pi` as `pie-pi-process`; a Node spawn path for pie-pi-process
