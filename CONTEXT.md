...
**Pi RPC child**:
Always Bun: `bun <pi-rpc.js> --mode rpc …`. `@getpie/server#build` emits the JS with `bun build --target bun`. Value imports go through `harness/pi/pi-sdk.ts` (not the package barrel). A pnpm patch keeps `pi-tui` on the extension virtualModules, points the loader at `sdk.js` instead of the InteractiveMode barrel, inlines builtin theme JSON, and no-ops highlight.js. Unpackaged / CLI look up `bun` on PATH. Packaged desktop ships Bun (`extraResources/vendor/bun`, currently `bun-v1.4.2`) and sets `PIE_BUN` / `PIE_PI_EXECUTABLE` to the extraResources copies. Missing Bun fails availability.
