...
**Pi RPC child**:
Always Bun: `bun <pi-rpc.js> --mode rpc …`. `@getpie/server#build` emits the JS with `bun build --target bun`. Value imports go through `harness/pi/pi-sdk.ts` (not the package barrel). A pnpm patch on the SDK only rewires modules that bun-build actually reaches: the extension loader, builtin theme JSON, tool renderers, and syntax-highlight. Unpackaged / CLI look up `bun` on PATH. Packaged desktop ships Bun (`extraResources/vendor/bun`, currently `bun-v1.4.2`) and sets `PIE_BUN` / `PIE_PI_EXECUTABLE` to the extraResources copies. Missing Bun fails availability.
