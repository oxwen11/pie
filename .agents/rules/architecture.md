...
| `tools/verify` | `@getpie/verify` (bin `pie-verify`) | Isolated proof helper for web / CLI / desktop. Surfaces: `pie-verify web|cli|desktop`. `web browser` / `desktop browser` wrap `agent-browser`. Not the product CLI. |
