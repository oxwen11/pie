...
Harness: the CLI itself plus `curl`. **No browser** — 4182 is the daemon/API, not the Vite SPA. Do not `pie-verify cli browser` / `pie-verify cli env`. UI proofs load `pie-verify web env` or `pie-verify desktop env`, then call `$AGENT_BROWSER`.
