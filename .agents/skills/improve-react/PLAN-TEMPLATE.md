...
- **Mechanical**:
  - From `apps/app`: `pnpm exec react-doctor --yes --verbose --scope changed`
    clears the targeted diagnostic and the score does not regress.
...
- Pull the exact fix from the canonical per-rule prompt at
  ...
  `pnpm exec react-doctor rules explain <rule>` (run from `apps/app`).
