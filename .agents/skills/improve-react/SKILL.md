...
This is a Vite + TanStack Query SPA, not Next.js RSC. Run the scanner from `apps/app` with the pinned CLI:

```bash
pnpm exec react-doctor --yes --json --json-out /tmp/react-doctor-report.json
```

Do not use `npx react-doctor@latest`. Respect `doctor.config.json` and `.agents/skills/react-doctor/references/strict-value.md` (Hard Rule 5): a rule we left off (`js-cache-property-access`, design tags, Next/RN) is a deliberate tradeoff, not a missed finding.

Write plans under `plans/` as this skill requires. Do not dump them into `todos/` unless the user asks to turn a plan into a numbered ticket. `todos/` is the existing security/perf remediation series.

AUDIT.md still lists some retired IDs (`no-many-boolean-props`). Those report nothing even if named — follow `.agents/skills/vercel-composition-patterns` for API shape instead of expecting Doctor to police boolean-prop counts.
...
Write for the weakest executor: exact file paths and current-code excerpts, the exact target code (pulled from the canonical per-rule prompt, never approximated), the repo's own conventions with an exemplar to imitate, ordered steps, hard scope boundaries, and a verification section — mechanical (`pnpm exec react-doctor --yes --verbose --scope changed` from `apps/app` clears the diagnostic without dropping the score, plus typecheck/lint/tests) and behavioral (what to click and what to confirm in the React DevTools Profiler / "Highlight updates").
