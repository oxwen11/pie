...
This is a Vite + TanStack Query SPA, not Next.js. Follow bundle, rerender, rendering, and `js-*` rules. Skip RSC / Server Action / `next/dynamic` / SWR rules (`server-*`, `bundle-dynamic-imports` as Next-only, `client-swr-dedup`). Server state stays in TanStack Query (`.agents/rules/topics/frontend-state.md`).
