...
- **CDP** — doctor attaches agent-browser (`connect` to `AGENT_BROWSER_CDP`). You can also attach yourself after `env --export`.
...
"$AGENT_BROWSER" --session "$AGENT_BROWSER_SESSION" get url
