...
agent-browser get title # starts recording.webm automatically at 60 fps
...
pnpm exec pie-verify desktop evidence note "recording.webm: what the clip shows"
pnpm exec pie-verify desktop evidence path
```

agent-browser 0.37.1 records the existing pinned renderer in place. The Verify shim starts one automatic `recording.webm` at 60 fps on the first browser command; do not call `record start`, `restart`, or `stop`. Cleanup flushes it before Electron exits. A green Playwright e2e run is not a substitute for the screenshots and video.
