...
- Builds `@getpie/server` (and thus `@getpie/core`) when `packages/server/dist/server.mjs` is missing. Desktop `dev` depends on that artifact (`apps/desktop/turbo.json`). Main launches the daemon as Electron-as-Node (`PIE_DAEMON_RUNTIME=node`) on `packages/server/dist/server.mjs`; packaged builds use the asar `server.mjs` plus `Contents/Resources/vendor/bun` only for `pie-pi-process`.
