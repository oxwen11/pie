...
| --- | --- | --- | --- | --- |
| Hub | `@getpie/hub` | `pie-hub` / `npx @getpie/hub` | GitHub webhooks, daemon WS, a tiny operator HTTP API | Import `@getpie/server` or `@getpie/cli`; load UI, oRPC, or `Project.path` |
...
`pie hub serve` would couple two install surfaces and make a VPS Hub pull
in the daemon, Pi, and local UI stack.

Shared code is `@getpie/contract` Hub schemas only. The Hub package may use
Effect platform layers of its own. It must not import harness, Project,
`http/main.ts`, or the CLI.
...
1. **CLI login** — human operator of Hub. Stored under `$PIE_HOME/hub/` keyed
   ...
   this only. This login lives on the **daemon** CLI because it enrolls the
   local machine; it is not a `pie-hub` command.
2. **Enrollment token** — one-time, ~10 minutes, `daemons:enroll`. The pie CLI
...
Hub is its own published package and its own CLI. The pie CLI never starts
Hub.
...
packages/hub                          @getpie/hub, bin pie-hub
  src/main.ts                         Hub composition root
  src/webhook.ts                      GitHub signature + event fold
  src/dispatch.ts                     match + pick relationship
  src/socket.ts                       inbound daemon WS
  src/store.ts                        PGlite / SQLite under $PIE_HUB_DATA
...
Start Hub the same way Paseo does — a separate installable:
...
Default bind is `:3000`. Data dir is `$PIE_HUB_DATA` or
`$XDG_DATA_HOME/pie-hub` (`~/.local/share/pie-hub`). Docker / Fly later wrap
this same binary; there is still no `pie hub serve`.

`HubConnector` starts only when a relationship file exists. The default
`pie` / `pie serve` path stays Hub-unaware until `connect`. Those `pie hub *`
commands configure the **daemon**. They must not depend on `@getpie/hub`.
...
No Hub dashboard in the SPA for V1. Operator checks `pie hub status` (daemon
enrollment) plus Hub process logs / the GitHub issue thread. A later Activity
view can read Hub's delivery log over the operator API.
...
4. **`@getpie/hub` + webhook** — new workspace package, `pie-hub` bin,
   signature verify, trigger match, delivery log, `daemon_not_connected`.
   Tests are HTTP fixtures, no Pi, no `@getpie/server` import.
5. **daemon CLI enroll** — `pie hub login|connect|status|disconnect` only.
   No `serve` subcommand.
6. **write-back** — 👀/🚀 reactions and a completion comment via `gh`.
7. **UI badge** — session list origin chip; screenshot on the PR.

Do not mix slice 4 (public Hub package) with slice 2 (session create). The
driver must be callable without `@getpie/hub` so it stays a session-domain
test.
...
**Hub as `pie hub serve` inside `@getpie/server`.** Couples the public ingress
to the daemon install. A VPS Hub would pull Pi, oRPC, and the UI stack; the
pie CLI would grow a second product. Architecture's "all runtime in server"
names the daemon, not every process. Follow Paseo's split: `@getpie/hub` +
`pie-hub` to start, pie CLI only to enroll.
...
1. One Hub organization vs many users on one `pie-hub` process — V1 assumes
