# Embedding the web app into Electron: opencode vs t3code vs pie
...
- **pie** — this repo, `apps/desktop`, as of PR #100
...
1. **What the protocol handler does** — serve files off disk (opencode, pie) or reverse-proxy the local server (t3code).
2. **How the server process starts** — Electron's `utilityProcess` (opencode) or a real child process on Electron's Node (t3code, pie).
3. **Who picks the port** — the host, before the server starts (opencode) or the server, reporting back (pie).

pie takes opencode's answer to (1), t3code's to (2), and its own to (3).
...
| **Window origin**  | `oc://renderer`                              | `t3code://` / `t3code-dev://`                        | `pie://app`                                  |
...
| **Readiness**      | MessagePort `ready` **+ HTTP health poll**   | ready flag + poll                                    | stdout `pie:ready {"port":N}`                |
...
| **CORS**           | allowlist `["oc://renderer"]`                | **wildcard `*`** — leans on the token, not origin    | allowlist `pie://app` + dev origin           |
...
**pie follows opencode**, and needs an **SPA fallback** where the others sidestep
...
owns routing and rendering, which t3code has and pie doesn't.
...
**pie follows t3code.** The decisive property: a packaged app needs no system
...
- **pie** inverts it: the server binds `:0` and prints `pie:ready {"port":N}` on stdout; the host parses that line. No race — the port is never released — at the cost of a small stdout protocol.
...
- **pie**: a **bearer** token, minted per launch and never written to disk, plus an exact CORS allowlist.

Worth pausing on the CORS split, because it's a real disagreement: opencode and
pie pin the allowed origin exactly; t3code opens it to `*` and treats the bearer
...
pie mints a single-use ticket over HTTP and passes it as `?ticket=`. Three
codebases, one constraint, three near-identical answers.

One thing pie does that neither peer does: the token reaches the server through
...
and pie's are effectively the same file.
...
`window.api` has ~35 methods. pie's is one synchronous call returning
`{httpBaseUrl, wsBaseUrl, token}` — the renderer gets its backend coordinates and
nothing else.

**Host detection is where opencode and t3code split, and pie sides with opencode.**
...
provide it through context. pie does exactly this with
...
same and adds a **`launchctl getenv PATH` fallback on darwin**. pie captures the
...
t3code and pie both drive the Claude Agent SDK, which execs a native `claude`
...
pie now does the same, by decision. Bundling would have cost ~230 MB and pinned
...
`ENOTDIR`. That was pie's silent 500 on `session/create`. The general rule:
...
electron-builder warns about exactly this. pie doesn't need it now that the
...
signer for Windows. pie has no Apple certificate yet, so it ad-hoc signs in an
...
## What pie should take next

- **opencode's two-signal readiness** — a process handshake _and_ an HTTP health poll. pie trusts the stdout line alone; a server that prints ready and then wedges would hang the window.
