# Server topology: every pie server is a discoverable single-instance daemon

How a `pie` server should be launched and reached — locally by the desktop app
and the `pie` CLI, and remotely by other machines. This is about **server
...
Today "the server started by desktop" and "the server started by `pie` CLI"
...
plain foreground process — it binds a port, serves, prints `pie:ready {port}`,
...
exactly: there, `pie serve` is a dumb foreground process and the _launch script_
...
- **Launcher (`resolveOrSpawnDaemon`, shared by CLI + desktop).** Read `daemon.pid`
  ...
  _resident_: a short-lived `pie` command must operate a backend that outlives it.
...
- **Shutdown policy (open).** A resident daemon needs a stop rule. Options: explicit
  only (`pie daemon stop`); idle-timeout guarded by active agent sessions
...
The existing `PIE_AUTH_TOKEN` → HTTP Bearer → WS-ticket chain stays as the
...
- **The unauthenticated "browser mode" is removed.** Today, an unset
  `PIE_AUTH_TOKEN` disables `/api/*` auth and the WS ticket
  (`packages/pie/src/node/server.ts:100`). Once a plain browser can reach the
...
- The remote server is the _same_ `pie serve`, already single-instance via its
  `ssh-launch` files — i.e. the daemon principle above, remote.

This design's only obligation to the remote plane is to keep `pie serve`
...
controls (Tailscale Funnel / Cloudflare Tunnel / ngrok) or a pie-hosted relay,
...
- **Never make an unauthenticated path load-bearing.** Today an unset
  `PIE_AUTH_TOKEN` disables auth entirely (`packages/pie/src/node/server.ts:100`).
...
- **"Server stays a library each launcher embeds" (per-launch instance).** Needs a
  lock anyway to avoid the `$PIE_HOME` split-brain, and diverges from the remote
...
2. **✅ Landed (CLI) — Add the daemon launcher (a layer above the server, not inside
   ...
   `pie serve` re-launched from this same CLI argv — no second bundle. Bare `pie`
   and `pie daemon {start,stop,status}` are the front-doors; brought forward from
   ...
   loopback are always trusted; extra origins ride ambient `PIE_CORS_ORIGINS`),
...
4. **Unify the launch seam.** Fold "launch + tunnel" (from the SSH design) into the
   ...
   servers are supervised identically. Add `pie daemon {start,stop,status}` and
