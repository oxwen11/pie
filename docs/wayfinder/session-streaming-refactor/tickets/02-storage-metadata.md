...
- `~/.pie` 目录布局：`storage/projects.json` 沿用还是调整？`storage/sessions/<projectId>/<sessionId>.json` 确认。
...
5. **server 侧新建编排层**：`packages/server` 新增 `SessionMetadataRepository` + `SessionService`，依赖保留的 `ProjectService`（projectId→path 解析）、生成 sessionId、写元数据，再调 harness 的 `HarnessAgentSessionService`；harness runtime 保持无盘、不知道 projectId。目录布局确认 `$PIE_HOME/storage/sessions/<projectId>/<sessionId>.json`。
