...
## Resource diagnostics (inventory reconciliation pending)

Implementation now exists under `packages/server/src/observability/resources/`,
`native/resource-monitor/`, and Desktop Main. The requirements below retain their
pending review status: this documentation cleanup did not verify their complete
host-write contract or runtime acceptance. Reconcile this inventory with the
implementation and evidence before treating it as an audited shipped contract.

[Resource monitoring design](rfc/resource-monitoring.md) proposes independent
...
implicitly start writers. Section 11 of that RFC links the existing composition
and writer modules and defines the outstanding product and packaged acceptance.
...
3–9 and require confirmation under section 12; existing code does not establish
that approval. Older releases ignore this new subtree; no business-data or
Pi-transcript migration is proposed. Reconcile the inventory with actual writes
and acceptance evidence before marking this contract reviewed.
