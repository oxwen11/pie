  ...
  return (
    <div className="flex flex-col gap-3 border-b p-3" aria-label="Linked pull requests">
      {projection.groups.map((group) => {
        const head = group.links[0];
        if (!head) return null;
        return (
          <section key={pullRequestKey(head.ref)} className="flex flex-col gap-1">
            ...
            {group.links.map((link) => {
              ...
              return (
                <div key={key} className="flex items-center gap-1">
                  <Button
                    className="h-auto min-w-0 flex-1 flex-col items-start text-start"
