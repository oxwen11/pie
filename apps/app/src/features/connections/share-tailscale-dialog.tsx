...
export function ShareTailscaleDialog({ onClose }: { onClose: () => void }): ReactElement {
  ...
  const [snapshotPromise, setSnapshotPromise] = useState(
    () => tailscale?.snapshot() ?? Promise.resolve(null),
  );
  const snapshot = use(snapshotPromise);
  const [pending, setPending] = useState(false);

  if (!tailscale || !tailscale.client.available || snapshot === null) return <></>;
        ...
        <DialogFooter>
          ...
          {snapshot.serveEnabled ? (
            ...
            <Button
              ...
              type="button"
              onClick={() =>
                run(() => tailscale.enableServe(), "Failed to enable Tailscale HTTPS.")
              }
            >
