...
describe("Chat compaction", () => {
  it("keeps live messages and appends the same turn's continuation as a new message", async () => {
    ...
    expect(assistantText(chat.store.getState().messages[1]!)).toBe("old text");
    expect(assistantText(chat.store.getState().messages[2]!)).toBe("new text");
  ...
  it.each(["canceled", "failed"] as const)(
    ...
    async (outcome) => {
      ...
      expect(assistantText(chat.store.getState().messages[0]!)).toBe("beforeafter");
  ...
  it("uses the compacted Session Messages only as the cold floor", async () => {
    ...
    expect(assistantText(chat.store.getState().messages[2]!)).toBe("continuing");
  });
