...
const toUserMessage = (messageId: string, parts: ReadonlyArray<PromptPart>): SessionUIMessage => ({
  id: messageId,
  role: "user",
...
const retryNoticeFrom = (chunk: SessionUIMessageChunk): string | undefined => {
...
type TurnFold = {
  readonly enqueue: (chunk: SessionUIMessageChunk) => void;
  readonly close: () => void;
};
...
export class Chat {
  ...
  #replayActiveTurn(activeTurn: NonNullable<SessionRuntimeSnapshot["activeTurn"]>): void {
    ...
    let chunks: SessionUIMessageChunk[];
  ...
  #observeChunk(chunk: SessionUIMessageChunk): void {
  ...
  #turnFold(turnId: string): TurnFold {
    ...
    let controller: ReadableStreamDefaultController<SessionUIMessageChunk> | undefined;
    const stream = new ReadableStream<SessionUIMessageChunk>({
      start(c) {
        controller = c;
      },
    });
    void (async () => {
      try {
        ...
        const seed = { id: `turn-${turnId}`, role: "assistant", parts: [] } as SessionUIMessage;
