...
import { hasPiAuth, PROJECT_ID, seedProject } from "./fixtures.js";
 ...
 *
 * Turns go through the real pie Pi process and a live model. A client attaching
 * after a turn ended still cannot backfill from history, so assertions are about
 * turns a client either observed live or recovered from the runtime snapshot.
 *
...
const createSession = async (browser: Browser, firstPrompt: string): Promise<Page> => {
  ...
  await expect(page.getByText(/PONG/i).first()).toBeVisible({ timeout: 120_000 });
  return page;
};
...
test("two clients on one session render each other's turns live", async ({ browser }) => {
  const pageA = await createSession(browser, "Reply with exactly: PONG-A1");
  const pageB = await joinSession(browser, pageA.url());

  // A → B: the prompt broadcast and the folded turn reach the observer.
  await send(pageA, "Reply with exactly: PONG-A2");
  await expect(pageB.getByText("Reply with exactly: PONG-A2")).toBeVisible({ timeout: 15_000 });
  await expect(pageB.getByText(/PONG-A2/i).first()).toBeVisible({ timeout: 120_000 });

  // B → A: same path in the other direction — no client is special.
  await send(pageB, "Reply with exactly: PONG-B1");
  await expect(pageA.getByText("Reply with exactly: PONG-B1")).toBeVisible({ timeout: 15_000 });
  await expect(pageA.getByText(/PONG-B1/i).first()).toBeVisible({ timeout: 120_000 });
  await expect(pageB.getByText(/PONG-B1/i).first()).toBeVisible({ timeout: 120_000 });
});
...
test("a disconnected client recovers a turn it missed, without a reload", async ({ browser }) => {
  const pageA = await createSession(browser, "Reply with exactly: PONG-WARM");
  ...
  await proxy.stop();
  await send(pageA, "Reply with exactly: PONG-OFF");
  await expect(pageA.getByText(/PONG-OFF/i).first()).toBeVisible({ timeout: 120_000 });
  await expect(pageB.getByText("Reply with exactly: PONG-OFF")).toBeHidden();
  ...
  await expect(pageB.getByText("Reply with exactly: PONG-OFF")).toBeVisible({ timeout: 30_000 });
  await expect(pageB.getByText(/PONG-OFF/i).first()).toBeVisible({ timeout: 120_000 });
  await revived.stop();
});
