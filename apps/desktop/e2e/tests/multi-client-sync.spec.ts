 ...
 * extension (no API key). Assertions stay on live/snapshot sync — history
 * backfill is a separate path.
 *
...
const joinSession = async (browser: Browser, sessionUrl: string): Promise<Page> => {
  ...
  // is in flight. With the real Pi process the join backfills turn history
  // from the session file, so "settled" means the full history is rendered.
...
test("two clients on one session render each other's turns live", async ({ browser }) => {
  ...
  // B → A: same path in the other direction — no client is special. Both
  // pages see all three replies: B backfilled turn 1 from the session file
  // when it joined, then turns 2–3 arrived live.
  ...
  await expect(pageB.getByText(FAKE_REPLY)).toHaveCount(3, { timeout: 15_000 });
});
