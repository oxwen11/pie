 ...
 * it can produce: instant single-message turns and no tool calls. That process
 * keeps the turns it already ran, so a client attaching later backfills them.
 *
...
const joinSession = async (browser: Browser, sessionUrl: string): Promise<Page> => {
  ...
  // is in flight. Fake-pi replays turns from this process, so a joiner sees them.
