...
export const makeSessionTurn = (deps: {
  ...
  const { manager, pi, repo, locks, newSessionId } = deps;
  const { readMetadata, ensureCwd, readAndStampTitleFromFirstPrompt } = deps.metadata;
