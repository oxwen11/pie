...
import type {
  ...
  RpcResponse as UpstreamRpcResponse,
  RpcSessionState as UpstreamRpcSessionState,
  SessionEntry,
  SessionStats,
  SessionTreeNode,
  SourceInfo,
...
type ThinkingLevel = UpstreamRpcSessionState["thinkingLevel"];
type Model<T = unknown> = NonNullable<UpstreamRpcSessionState["model"]> & T;
type ImageContent = NonNullable<Extract<UpstreamRpcCommand, { type: "prompt" }>["images"]>[number];
type BashResult = Extract<UpstreamRpcResponse, { command: "bash"; success: true }>["data"];
