...
export default defineConfig([
  ...
  {
    // Per-session Pi child. Pi stays external so getPackageDir/wasm/jiti keep
    // resolving files inside node_modules/@earendil-works/pi-coding-agent.
    entry: { "pi-process": "src/harness/pi/process-host/main.ts" },
    platform: "node",
    format: ["esm"],
    deps: {
      neverBundle: [/^@earendil-works\/pi-/],
    },
    dts: false,
    clean: false,
    shims: true,
